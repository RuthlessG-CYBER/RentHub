      ...
      <main className="flex-1 flex flex-col">
        ...
        <section className="p-8">
          ...
          {activeSection === "applications" && (
            <ApplicationsSection bookings={bookings} />
          )}
...
function LandlordDashboard({
  ...
  const accept = async (bookingId: string) => {
    await axios.post(
      `http://localhost:4040/api/user/bookings/${owner.id}/${bookingId}/accept`
    );
  ...
  const reject = async (bookingId: string) => {
    await axios.post(
      `http://localhost:4040/api/user/bookings/${owner.id}/${bookingId}/reject`
    );
    window.location.reload();
  };
...
function ApplicationsSection({ bookings }: { bookings: BookingType[] }) {
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-slate-900">
...
function NotificationsSection({
  ...
  const markRead = async (id: string) => {
    await axios.put(
      `http://localhost:4040/api/user/${userId}/notifications/${id}/read`
    );
    window.location.reload();
  };
